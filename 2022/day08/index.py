...
with open("input.txt", "r", encoding="utf-8") as f:
    data_list = [list(map(int, s)) for s in f.read().splitlines()]
...
for j in range(1, J - 1):
    for i in range(1, I - 1):
        ...
        view_left = np.logical_and.accumulate(larger_left).sum()
        view_left += (~larger_left).any()

        larger_right = height > data[j, i + 1 :]
        view_right = np.logical_and.accumulate(larger_right).sum()
        view_right += (~larger_right).any()

        larger_top = height > data[:j, i][::-1]
        view_top = np.logical_and.accumulate(larger_top).sum()
        view_top += (~larger_top).any()

        larger_bottom = height > data[j + 1 :, i]
        view_bottom = np.logical_and.accumulate(larger_bottom).sum()
        view_bottom += (~larger_bottom).any()
